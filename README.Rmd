...
In general compression utilities such as zip, gzip do not compress short strings well and 
often expand them. `strzip` wraps two short string compression C libraries:

- SMAZ: <https://github.com/antirez/smaz> (`str_compress_raw()`)
- Unishox2 <https://github.com/siara-cc/Unishox2> (`str_compress()`)

to compress short strings. In practice, `str_compress()` is safer to use and also comes with 
a S3 class `strzip` with its own methods.
